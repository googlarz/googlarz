...
| [codebase-onboarding](https://github.com/googlarz/codebase-onboarding) | Six-mode codebase investigator — maps architecture and external integrations (Stripe, S3, queues — what needs mocking locally), hunts undocumented gotchas including flaky tests, generates a local dev guide and week-by-week ramp-up plan. Then stays useful: risk-check a file before touching it with a scoped Claude prompt, generate a PR description draft + full preflight before pushing, or map a ticket to files and conventions before writing a line. Degrades gracefully without CODEBASE.md. |
