...
| [codebase-onboarding](https://github.com/googlarz/codebase-onboarding) | Runs a codebase investigation before you touch anything — entry points, git-history conventions, danger zones by churn and reverts, undocumented env vars, external integrations that need mocking. Produces CODEBASE.md with confidence tags (verified, inferred, gap); technical users also get a local dev guide and week-by-week ramp-up plan derived from actual findings; non-technical users get plain language, a shareable architecture diagram, and an executive brief. Then stays live: touch mode risk-checks a file before you edit it, preflight drafts your PR description and flags violations, task mode maps a ticket to the right files and reviewer. |
