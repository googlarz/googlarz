...
| | |
|---|---|
| [vibe-safe](https://github.com/googlarz/vibe-safe) | Active session guardian for non-technical contributors (PMs, designers) shipping AI-assisted code in shared codebases. 40 risk categories: credentials outside your diff, quality gate weakening, XSS sinks, SSL bypass, SQL injection, shell injection, Math.random() in security contexts, private key files, and more. Developer-defined contracts via `.vibesafe`. Pre-commit hook + CI integration — every flag cites file:line. |
